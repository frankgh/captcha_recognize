...
def _conv2d(value, weight):
    ...
    return tf.nn.conv2d(input=value, filters=weight, strides=[1, 1, 1, 1], padding='SAME')
...
def _weight_variable(name, shape):
    """weight_variable generates a weight variable of a given shape."""
    with tf.device('/cpu:0'):
        var = tf.Variable(tf.random.truncated_normal(shape, stddev=0.1), name=name, dtype=tf.float32)
    return var
...
def _bias_variable(name, shape):
    """bias_variable generates a bias variable of a given shape."""
    with tf.device('/cpu:0'):
        var = tf.Variable(tf.constant(0.1, shape=shape), name=name, dtype=tf.float32, shape=shape)
    return var
...
def inference(images, keep_prob):
    ...
    local1_drop = tf.nn.dropout(local1, 1 - (keep_prob))
...
def loss(logits, labels):
    cross_entropy = tf.nn.softmax_cross_entropy_with_logits(
        labels=tf.stop_gradient(labels), logits=logits, name='cross_entropy_per_example')
...
def evaluation(logits, labels):
    correct_prediction = tf.equal(tf.argmax(input=logits, axis=2), tf.argmax(input=labels, axis=2))
    correct_batch = tf.reduce_mean(input_tensor=tf.cast(correct_prediction, tf.int32), axis=1)
    return tf.reduce_sum(input_tensor=tf.cast(correct_batch, tf.float32))


def output(logits):
    return tf.argmax(input=logits, axis=2)
